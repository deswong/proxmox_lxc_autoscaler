...
def test_predictor():
    ...
    assert predictions['cpu_percent'] == 75.0, "CPU Prediction fallback should match the latest metric"
    assert predictions['ram_usage_mb'] == 1024.0, "RAM Prediction fallback should match the latest metric"
    ...
    assert predictions_fallback['cpu_percent'] == 15.0, "CPU Prediction fallback should match the latest metric"
    assert predictions_fallback['ram_usage_mb'] == 512.0, "RAM Prediction fallback should match the latest metric"
...
def test_scaler():
    ...
    current_metrics = {"allocated_cpus": 2, "allocated_ram_mb": 2048.0, "cpu_percent": 80.0}
    ...
    assert px.last_update is None, "Scaler should abort the API call since all requested scale-ups were denied by the safety cap!"
